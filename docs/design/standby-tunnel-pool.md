...
| goroutines, per route group | **7 unconditional** service loops — keep-alive 5 min, leg-liveness 30 s, leg-dataprogress 5 s, **send-window 100 ms**, reorder-stall 500 ms, legstate-resync 7 s, unidir-flip 1 s (`route_group.go:1982-2018`) — plus sack 2.5 min, tlp 100 ms, **fec-flush 50 ms** on negotiation, plus rotation 20 s **dial side only**. Initiator ≈ 10–11, exit ≈ 9–10. No per-group read loop (reads are per-transport) | |
| exit-side, per accepted tunnel | the above **+ 1 `yamux.Server` session (2 goroutines) + 1 `socks.Serve` goroutine** ⇒ ≈ **12–13 goroutines**. `MaxStreamWindowSize` 16 MiB is a flow-control ceiling, **not** an allocation | `pkg/skysocks/server.go:121-135`, `client.go:46` |
