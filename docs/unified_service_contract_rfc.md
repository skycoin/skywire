...
3. **`tcp`** — TCP-standalone (the "ssh equivalent"). Listens on a TCP port. Each accept runs a **Noise XK handshake** for mutual PK auth + an encrypted+integrity-checked channel — same crypto primitive dmsg uses, transported over raw TCP. The primitive lives at `pkg/dmsg/tcpnoise/` (factored out of `pkg/dmsg/dmsgpty/`) and is already wired by skychat's TCP-direct mode (`cmd/apps/skychat/commands/tcp_direct.go`). The pty subsystem just needs to use the same package — no new crypto, no new protocol.
