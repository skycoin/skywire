...
##### DHT full node — removed

This section described `enable_dht`, `redis_addr` and `persist_path` turning
the dmsg-server into a Kademlia DHT full node on dmsg port 100. **That feature
no longer exists**: there is no `pkg/dht`, no `dht` CLI subcommand and no
`dht:*` Redis keyspace, and nothing reads those fields. Discovery is served by
CXO and the HTTP discovery services.

Drop `enable_dht`, `redis_addr` and `persist_path` from any dmsg-server
`config.json` you are carrying forward. The dmsg-server needs no Redis, no
`REDIS_PASSWORD` in its unit, and no state directory.
