...
#### DMSG server DHT — removed

Earlier revisions of this document described `enable_dht: true` and
`redis_addr` turning a dmsg-server into a Kademlia DHT full node on dmsg port
100, and recommended it for production. **That feature no longer exists.**
There is no `pkg/dht`, no `dht` CLI subcommand, no `dht:*` Redis mirror, and
nothing reads `enable_dht` — setting it does nothing.

Discovery is served by CXO and by the HTTP discovery services. If you are
copying an old `config.json`, drop `enable_dht` and `redis_addr` from the
dmsg-server stanza; the dmsg-server needs no Redis of its own.
