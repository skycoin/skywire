...
|---|---|---|
| VPN consent + tunnel | `VpnService` + `VpnService.Builder` (`SkyVpnService.kt`) | The system owns `/dev/net/tun`; an app never opens it. The TUN arrives as a file descriptor over a unix socket via `SCM_RIGHTS` — see the protocol in [`pkg/vpn/tun_device_android.go`](https://github.com/skycoin/skywire/blob/develop/pkg/vpn/tun_device_android.go). No fds, no unix sockets, no service binding in a WebView. |
...
`/api/*` surface — [`hypervisor_handlers_browse_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/hypervisor_handlers_browse_mobile.go)
...
Angular assets in [`static_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/static_mobile.go). Shipping
...
| File | Reason |
|---|---|
| [`geoip_embedded_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/geoip/geoip_embedded_mobile.go) | Drops the ~30 MB embedded GeoLite2-City database. Size. |
| [`hypervisor_handlers_wallet_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/hypervisor_handlers_wallet_mobile.go) | Drops the ~11 MB vendored skycoin-web bundle; the phone wallet is native app-side code. Size. |
| [`hypervisor_dmsg_ingest_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/hypervisor_dmsg_ingest_mobile.go) | Remote-visor dmsg ingest is off unless the user opts in via Fleet. Battery. |
| [`survey_whitelist_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/visorconfig/survey_whitelist_mobile.go) | The deployment survey whitelist is not applied. Those keys let holders pull logs, system survey and pprof over dmsg; an operator's machine is that machine, a handset is not. Threat model. |
...
- [`hypervisor_tpviz_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/hypervisor_tpviz_mobile.go) —
...
- [`hypervisor_handlers_browse_mobile.go`](https://github.com/skycoin/skywire/blob/develop/pkg/visor/hypervisor_handlers_browse_mobile.go) —
...
- **`.dmsg` / `.skynet` name resolution under the VPN.** Not working on Android.
  [`mesh_client_other.go`](https://github.com/skycoin/skywire/blob/develop/pkg/vpn/mesh_client_other.go) is a stub and the
