...
| `HVDESKADDR` | `:8010` | Desk (wasm-visor hypervisor UI) listen address, served beside the dashboard on `HVHTTPADDR` (no wasm visor in the page). Default false = the desk. Runtime: skywire cli visor hv enable --legacy[=false] -w |
