...
// Package call pkg/skychat/call/audio_wasm.go c4-app-chat
//
...
//
//	CAPTURE  main thread mic → {t:'voiceMic'} → worker.js calls __skycallMic(bytes)
//	         → this Source's ring → session sendLoop.
//	PLAYBACK session recvLoop → this Sink → self.__skycallEmit(bytes)
//	         → worker.js posts {t:'voicePlay'} → main thread plays.
//
// In the in-page (single-file) visor, worker.js isn't involved: voice-audio.js
// defines __skycallEmit/__skycallMic on the same global and the calls are
...
// installBridge registers __skycallMic — the function the capture side calls to
// deliver one mic frame (LE int16 bytes) to the current call's Source.
func installBridge() {
	js.Global().Set("__skycallMic", js.FuncOf(func(_ js.Value, args []js.Value) any {
		if len(args) < 1 {
			return nil
		}
		u8 := args[0]
...
// via the __skycallEmit hook (defined by worker.js in worker mode, or
...
func (s *wasmSink) Write(pcm []int16) (int, error) {
	emit := js.Global().Get("__skycallEmit")
