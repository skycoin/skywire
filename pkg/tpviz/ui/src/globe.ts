...
function createEarthTexture(): THREE.Texture {
    ...
    ctx.beginPath();
    let [x, y] = toCanvas(-168, 65);
    ctx.moveTo(x, y);
    [[- 140, 70], [-130, 72], [-100, 73], [-85, 70], [-75, 60], [-55, 50], [-67, 45],
     [-70, 43], [-75, 35], [-80, 25], [-90, 20], [-100, 20], [-105, 22],
     [-117, 32], [-125, 40], [-125, 48], [-135, 55], [-150, 60], [-165, 65], [-168, 65]
    ].forEach(([lon, lat]) => {
        [x, y] = toCanvas(lon, lat);
        ctx.lineTo(x, y);
    });
    ctx.closePath();
    ctx.fill();
    ...
    [x, y] = toCanvas(-80, 10);
    ctx.moveTo(x, y);
    [[-60, 5], [-35, -5], [-35, -20], [-40, -23], [-45, -23], [-50, -25],
     [-55, -30], [-58, -38], [-65, -42], [-68, -52], [-75, -50], [-73, -40],
     [-70, -30], [-70, -18], [-75, -5], [-80, 0], [-80, 10]
    ].forEach(([lon, lat]) => {
        [x, y] = toCanvas(lon, lat);
        ctx.lineTo(x, y);
    });
    ...
    [x, y] = toCanvas(-10, 36);
    ctx.moveTo(x, y);
    [[-10, 44], [-5, 48], [0, 50], [5, 51], [10, 54], [15, 55], [25, 55],
     [30, 60], [28, 70], [20, 70], [10, 62], [5, 60], [-5, 58], [-10, 50],
     [-10, 36]
    ].forEach(([lon, lat]) => {
        [x, y] = toCanvas(lon, lat);
        ctx.lineTo(x, y);
    ...
    [x, y] = toCanvas(-17, 15);
    ctx.moveTo(x, y);
    [[-15, 28], [-5, 36], [10, 37], [25, 32], [35, 30], [40, 12],
     [50, 12], [50, 0], [42, -12], [35, -22], [28, -33], [18, -35],
     [15, -30], [12, -17], [10, -5], [0, 5], [-5, 5], [-17, 15]
    ].forEach(([lon, lat]) => {
        [x, y] = toCanvas(lon, lat);
        ctx.lineTo(x, y);
    });
    ...
    [x, y] = toCanvas(30, 40);
    ctx.moveTo(x, y);
    [[40, 42], [50, 45], [60, 55], [70, 55], [80, 50], [90, 45],
     [100, 35], [110, 35], [120, 40], [130, 45], [140, 45], [145, 50],
     [150, 60], [180, 65], [180, 70], [130, 75], [100, 75], [70, 70],
     [55, 55], [40, 45], [30, 40]
    ].forEach(([lon, lat]) => {
        [x, y] = toCanvas(lon, lat);
        ctx.lineTo(x, y);
    ...
    [x, y] = toCanvas(114, -22);
    ctx.moveTo(x, y);
    [[120, -18], [130, -12], [142, -11], [150, -15], [153, -25],
...
export function updateGlobeData(): void {
    ...
    nodes.forEach((node: any) => {
        ...
        // Skip offline/unknown based on filters
