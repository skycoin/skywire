...
func (_m *MockIDReserver) PopID(pk cipher.PubKey) (routing.RouteID, bool) {
	...
	if len(ret) == 0 {
		panic("no return value specified for PopID")
	}
...
func (_m *MockIDReserver) ReserveIDs(ctx context.Context) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for ReserveIDs")
	}
...
// TotalIDs provides a mock function with no fields
func (_m *MockIDReserver) TotalIDs() int {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for TotalIDs")
	}
