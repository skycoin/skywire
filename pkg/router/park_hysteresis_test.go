...
// that floor is withheld — see TestSBDNoParkWithoutTrafficToRuleOn.
func loadedDeltas(mts []*transport.ManagedTransport) map[uuid.UUID]uint64 {
	out := make(map[uuid.UUID]uint64, len(mts))
	for _, mt := range mts {
		out[mt.Entry.ID] = 5_000_000
	}
	return out
}
...
func TestAdaptiveParkHoldStopsControllerFlap(t *testing.T) {
	...
	rg.enforceBottleneckGroups(loadedDeltas(mts))
