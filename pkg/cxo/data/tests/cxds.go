// Package tests pkg/cxo/data/tests/cxds.go c2-net-cxo
package tests
...
func CXDSGet(t *testing.T, ds data.CXDS) {
	...
	if _, err := ds.Set(key, value, 1); err != nil { //nolint:errcheck,gosec
		t.Error(err)
		return
	}
...
func CXDSSet(t *testing.T, ds data.CXDS) {
	...
	t.Run("zero", func(t *testing.T) {
		defer shouldPanic(t)
		ds.Set(key, value, 0) //nolint:errcheck,gosec
	})

	t.Run("negaive", func(t *testing.T) {
		defer shouldPanic(t)
		ds.Set(key, value, -1) //nolint:errcheck,gosec
	})

	t.Run("new", func(t *testing.T) {
		if rc, err := ds.Set(key, value, 1); err != nil { //nolint:errcheck,gosec
			t.Error(err)
		} else if rc != 1 {
			t.Error("wrong rc", rc)
		}
	...
	t.Run("three times", func(t *testing.T) {
		if rc, err := ds.Set(key, value, 2); err != nil { //nolint:errcheck,gosec
			t.Error(err)
		} else if rc != 4 {
			t.Error("wrong rc", rc)
		}
