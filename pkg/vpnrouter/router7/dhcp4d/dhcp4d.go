...
func NewHandler(dir string, iface *net.Interface, ifaceName string, conn net.PacketConn) (*Handler, error) {
	...
	// host-OS port: the DHCP server address is the interface's own assigned
	// IPv4 (the LAN gateway address). Upstream router7 read this from its
	// interfaces.json via netconfig.LinkAddress; using the live interface
	// address instead drops the heavy netconfig -> nftables/netlink/wireguard
	// dependency tree, which the DHCP server does not otherwise need.
	serverIP, err := ifaceFirstIPv4(iface)
	if err != nil {
		return nil, err
	}
	if conn == nil {
...
// ifaceFirstIPv4 returns the first IPv4 address assigned to iface.
