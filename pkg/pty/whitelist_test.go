...
func TestWhitelistClientGateway_Integration(t *testing.T) {
	...
	// Client side.
	wlClient, err := pty.NewWhitelistClient(connClient)
	require.NoError(t, err)
...
func TestRPCUtil_RejectResponse(t *testing.T) {
	...
	_, err := pty.NewWhitelistClient(connB)
	require.Error(t, err)
...
func TestWriteConfig(t *testing.T) {
	...
	err := pty.WriteConfig(c, confPath)
	require.NoError(t, err)
...
func TestParseWindowsEnv_NonWindows(t *testing.T) {
	...
	input := "%HOMEDRIVE%%HOMEPATH%\\pty.sock"
	result := pty.ParseWindowsEnv(input)
