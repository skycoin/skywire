...
// and PublicVisorUpdater live here (native-only). The Updater interface and the
// no-op emptyUpdater stay in discovery_manager.go; on TinyGo the Factory hands
// out emptyUpdater (a browser visor does not register in service discovery).
package appdisc
