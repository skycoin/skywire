...
func TestMockRPCIngressClient_AllMethods(t *testing.T) {
	...
	c.On("DialWithOptions", DialOptionsReq{Addr: addr, MuxRoutes: 1, ForwardMuxRoutes: 1, ReverseMuxRoutes: 1}).Return(uint16(3), routing.Port(4), nil)
	...
	_, _, err = c.DialWithOptions(DialOptionsReq{Addr: addr, MuxRoutes: 1, ForwardMuxRoutes: 1, ReverseMuxRoutes: 1})
	require.NoError(t, err)
