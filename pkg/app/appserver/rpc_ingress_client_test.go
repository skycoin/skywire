...
func TestRPCIngressClient_DialWithOptions(t *testing.T) {
	t.Run("ok with muxRoutes", func(t *testing.T) {
		...
		connID, localPort, err := rpcC.DialWithOptions(DialOptionsReq{Addr: remote, MuxRoutes: 4})
		require.NoError(t, err)
	...
	t.Run("ok with minHops", func(t *testing.T) {
		...
		connID, localPort, err := rpcC.DialWithOptions(DialOptionsReq{Addr: remote, MinHops: 2})
		require.NoError(t, err)
		require.Equal(t, uint16(1), connID)
		require.Equal(t, routing.Port(dmsgLocal.Port), localPort)
	})
