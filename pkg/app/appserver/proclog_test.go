// Package appserver pkg/app/appserver/proclog_test.go c2-vis-appsvc
package appserver
...
// TestAppLogHook_LevelsShareOneWriter pins the bug that lost every INFO, WARN
// and ERROR line from an app's skywire.log. With a lumberjack writer per level
// over the same path, DEBUG volume rotated the file out from under the other
// levels and they went on writing to the renamed inode. One writer means an
// Info line logged after a rotation is in the file that is actually on disk.
func TestAppLogHook_LevelsShareOneWriter(t *testing.T) {
	...
	// the rotation race in a running app.
