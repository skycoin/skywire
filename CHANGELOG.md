...
-   fix(router): a SACK-requested retransmit honours the sending leg's delay basis — the ceiling clamp that made a queue-deep leg wait exactly one mean delay is gone, young frames are deferred and counted as `retx_deferred_young` (a 10 MB two-leg upload carried 14.6 MB on the wire)  [#5032](https://github.com/skycoin/skywire/pull/5032)
-   fix(skysocks): striped-upload chunks are placed by upload capacity and RTT, an unmeasured tunnel is credited the median, and an object admitted in one burst keeps its tail chunk on the fastest tunnel (`upload.burst_plan`)  [#5031](https://github.com/skycoin/skywire/pull/5031)
-   fix(router): an acceptor never parks a stalled leg on its own — it records `leg_stalled` and the active set stays the initiator's mirror  [#5030](https://github.com/skycoin/skywire/pull/5030)
-   fix(skysocks,appserver): a head-of-line-blocked tunnel is not snubbed (`tunnel.snub_after` 3 s → 20 s, above the longest measured reorder-wedge clear) and snub events reach the visor's mux-event log  [#5029](https://github.com/skycoin/skywire/pull/5029)
-   bench: results and READMEs for the 2026-09-18 chains, and the criterion status table  [#5028](https://github.com/skycoin/skywire/pull/5028)
-   fix(skysocks): a striped upload survives a chunk the client itself abandoned — the sink's short-chunk 400 is retried once (`upload.cut_tries`), never terminal  [#5027](https://github.com/skycoin/skywire/pull/5027)
-   fix(skysocks),bench: a failed striped upload says why (502 body + `X-Upload-Error`), the app's own warnings reach the visor log, the bench keeps a failed row's headers and body, and the blackout capture reads the live visor log  [#5025](https://github.com/skycoin/skywire/pull/5025)
-   fix(router): shared-bottleneck detection samples per-leg newly-acked frames (not the frontier purge), a park needs traffic evidence, is a goodput trial with backoff when enabled, and demotion is OFF by default — every ruling is recorded as `sbd_ruling`; RACK uses each leg's own end-to-end delay basis (legs-2 at 50 MB: x1.13 with parking off vs x0.86 with it)  [#5012](https://github.com/skycoin/skywire/pull/5012)
-   feat(skysocks): a silent tunnel is snubbed and its chunks re-issued elsewhere; per-tunnel queue depth follows the bandwidth-delay product; the spread policy skips a snubbed tunnel  [#5013](https://github.com/skycoin/skywire/pull/5013)
-   feat(skysocks): a spread policy as live knobs — `spread.max_share` caps any one route's share of the bytes, `spread.min_routes` promotes standbys until that many routes carry, chunks are assigned in proportion to measured capacity with pick and reservation in one critical section, the download in-flight budget scales with the active tunnels (`chunk.tunnel_concurrency`), and `min_routes` promotes the highest-capacity standby (live: 3 routes, top share 0.44, 0.87x/0.95x the best single route at 50 MB)  [#5014](https://github.com/skycoin/skywire/pull/5014)
-   fix(skysocks): the range splitter streams the frontier chunk as it arrives, so a mid-transfer cut of the active tunnel costs one detection, not one chunk (time to first byte after the cut 6.8 s → 0.64 s)  [#5023](https://github.com/skycoin/skywire/pull/5023)
-   fix(skysocks): a standby's audition never takes the entry stream, so chunk0 stays on an active tunnel (two of five 50 MB downloads had collapsed to the direct rate)  [#5022](https://github.com/skycoin/skywire/pull/5022)
-   chore: golangci-lint findings cleared (misspell, unparam, unused) — a red lint lane is real from here on  [#5021](https://github.com/skycoin/skywire/pull/5021)
-   fix(router): every route group drains its own inbound intake queue on its own worker, so one group whose app stopped reading no longer parks the visor's single transport read loop for 30 s and blacks out every other group; `visor state --select diag` shows each group's intake queue and drops  [#5018](https://github.com/skycoin/skywire/pull/5018)
