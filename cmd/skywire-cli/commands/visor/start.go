...
func init() {
	...
	if root {
		...
		startCmd.Flags().BoolVarP(&testing, "test", "t", false, "'go run' external commands from the skywire sources")
	}
}
...
var startCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, args []string) {
		...
		if !testing {
			...
			_, err = script.Exec(`go run cmd/skywire-visor/skywire-visor.go`).Stdout()
		}
		if err != nil {
