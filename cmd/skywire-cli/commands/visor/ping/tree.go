...
Use --dmsg-all-servers to ping via all DMSG servers (not just first success).`
	...
	pingTreeCmd.Flags().IntVar(&treeConcurrency, "concurrency", 2, "max concurrent ping operations")
...
var pingTreeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		graphConcurrency = treeConcurrency

		// Create RPC client first so cleanup can use it
