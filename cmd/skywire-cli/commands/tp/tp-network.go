...
func init() {
	networkCmd.Flags().StringVarP(&nSDURL, "sdurl", "a", deployment.Prod.ServiceDiscovery, "service discovery url")
	networkCmd.Flags().StringVarP(&nTPDURL, "tpdurl", "b", deployment.Prod.TransportDiscovery, "transport discovery url")
	networkCmd.Flags().StringVar(&nCacheFileSD, "cfs", os.TempDir()+"/networksd.json", "SD cache file location")
	networkCmd.Flags().StringVar(&nCacheFileTPD, "cft", os.TempDir()+"/networktpd.json", "TPD cache file location")
	networkCmd.Flags().IntVarP(&nCacheFilesAge, "cfa", "m", 5, "update cache files if older than n minutes")
	networkCmd.Flags().StringVarP(&nCountry, "country", "c", "", "filter by country code")
	networkCmd.Flags().StringVarP(&nVersion, "version", "e", "", "filter by version")
	networkCmd.Flags().IntVarP(&nMinTransports, "min", "n", 0, "filter by minimum transport count")
	networkCmd.Flags().BoolVar(&nJSONOutput, internal.JSONString, false, "print output in json")
}

var networkCmd = &cobra.Command{
	Use:   "network",
	Short: "Show network-wide statistics",
...
Shows public keys with their services and transport counts by type.`,
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		proxyData := internal.GetData(nCacheFileSD+"_proxy", nSDURL+"/api/services?type="+servicedisc.ServiceTypeProxy, nCacheFilesAge)
		vpnData := internal.GetData(nCacheFileSD+"_vpn", nSDURL+"/api/services?type="+servicedisc.ServiceTypeVPN, nCacheFilesAge)
		visorData := internal.GetData(nCacheFileSD+"_visor", nSDURL+"/api/services?type="+servicedisc.ServiceTypeVisor, nCacheFilesAge)

		// Fetch transport discovery data
		tpdData := internal.GetData(nCacheFileTPD, nTPDURL+"/all-transports", nCacheFilesAge)
		...
		for _, e := range visorEntries {
			pk := e.Address // Visor address is just the public key
			if serviceMap[pk] == nil {
				serviceMap[pk] = &serviceInfo{PK: pk, Country: e.Geo.Country, Version: e.Version}
			}
		...
		for pk, info := range serviceMap {
			...
			// Apply filters
			if nCountry != "" && info.Country != nCountry {
				continue
			}
			if nVersion != "" && info.Version != nVersion {
				continue
			}
			if nMinTransports > 0 && counts.Total < nMinTransports {
				continue
			}
