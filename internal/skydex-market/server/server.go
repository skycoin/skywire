...
// frame back. See skydex-design.md §8.
package server
...
// connections. Both *app.Client and the skydex-market app wrapper satisfy it.
...
func (s *Server) ListenAndServe(ctx context.Context, l Listener) error {
	...
	s.log.Infof("skydex-market: listening on dmsg port %d", s.port)
...
func (s *Server) serveConn(conn net.Conn) {
	...
	if !ok {
		s.log.Warnf("skydex-market: rejecting conn with non-appnet remote addr %v", conn.RemoteAddr())
		return
	}
	pk := raddr.PubKey.Hex()
	...
	if !s.acquire(pk) {
		...
		s.log.Warnf("skydex-market: rejecting duplicate session for %s", pk)
		return
	}
	defer s.release(pk)

	s.log.Debugf("skydex-market: session opened for %s", pk)
	s.Serve(conn, pk)
	s.log.Debugf("skydex-market: session closed for %s", pk)
}
