...
func TestEnv_DmsgWeb(t *testing.T) {
	...
	// but start it explicitly too so the test is robust to launcher timing. The
	// command is idempotent enough — an "already running" error is harmless, and
	// the curl retry below is the real readiness gate.
	if _, err := env.Exec("/release/skywire cli --rpc " + visorB + ":3435 visor app start dmsgweb"); err != nil {
		t.Logf("visor app start dmsgweb (non-fatal, may already be running): %v", err)
	}

	// Fetch the AR's /health over dmsg via the SOCKS5 resolver. Retry to absorb
	// resolver startup + a cold dmsg session to the AR.
	url := fmt.Sprintf("http://%s.dmsg:80/health", arDmsgPK)
	cmd := fmt.Sprintf("curl -s -m 25 -x %s %s", dmsgWebProxy, url)
	...
	require.Eventually(t, func() bool {
		...
		"dmsgweb did not return the address-resolver /health over dmsg (last body: %.200q)", body)
