...
func SetupTUN(ifcName, ipCIDR, gateway string, mtu int) error {
	if err := osutil.RunElevated("ip", "a", "add", ipCIDR, "dev", ifcName); err != nil {
		return fmt.Errorf("error assigning IP: %w", err)
	}

	if err := osutil.RunElevated("ip", "link", "set", "dev", ifcName, "mtu", strconv.Itoa(mtu)); err != nil {
	...
	if err := osutil.RunElevated("ip", "link", "set", ifcName, "up"); err != nil {
...
func DeleteRoute(ip, gateway string) error {
	return osutil.RunElevated("ip", "r", "del", ip, "via", gateway)
}
