...
// TestSkydexTradeOverDmsg runs list -> deposit -> buy -> pay -> confirm ->
// deliver across a real dmsg mesh with three identities.
func TestSkydexTradeOverDmsg(t *testing.T) {
	if testing.Short() {
	...
	// escrow seed; availability now requires node + seed + address).
	...
	srv := server.New(database, nil, marketPort)
